...
Este projeto segue o mesmo modelo das integrações Azure e AWS e utiliza o inventário já coletado pelo **Ansible Automation Platform (AAP)** como fonte de dados para cadastro dos ativos no CMDB.

---

# ⚠️ Escopo Atual do Projeto

Embora a arquitetura já possua componentes para operações de **Create**, **Update** e **Deactivate**, atualmente o projeto encontra-se homologado e aprovado apenas para **criação de novos ativos** no CMDB.
...
A estratégia adotada nesta primeira fase visa evitar alterações automáticas ou desativações indevidas enquanto as regras de governança e ownership dos ativos são consolidadas.
...
O projeto não consulta diretamente o GCP durante o processo normal de sincronização.
...
Playbook opcional utilizado para habilitar sincronização baseada em eventos do GCP.
...
Isso preserva qualquer ajuste manual realizado pelas equipes responsáveis.
