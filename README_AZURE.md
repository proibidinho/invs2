...
Sincronização de inventário **Azure** (via Ansible Automation Platform)
para **Jira Assets / CMDB**.

O projeto utiliza o inventário Azure sincronizado no **Ansible
Automation Platform (AAP)** como fonte oficial de dados para cadastro de
servidores no CMDB.

Seguindo o mesmo padrão adotado para AWS e GCP, todas as informações
consumidas pelo processo são obtidas diretamente do inventário do AAP,
evitando consultas adicionais à Azure durante o sync.
...
Dessa forma não é necessário executar consultas adicionais na Azure
durante o sync.
...
## Campos Populados no CMDB
  ...
  FQDN (3343)                                  public_dns_hostnames\[0\] ou fallback
                                               name-vmid

  Sistema Operacional (3358)                   os_disk.operating_system_type /
                                               image.offer

  Modelo do Servidor (15656)                   virtual_machine_size →
                                               modelo_servidor_map

  CPU Count (3359)                             azure_vm_specs\[vm_size\].cpu

  Memória RAM (6840)                           azure_vm_specs\[vm_size\].memory_gb
                                               \* 1024
...
## Script auxiliar `scripts/coleta_azure.py`

Assim como o `coleta_aws.py`, o `coleta_azure.py` **não participa da
sincronização AAP → CMDB**. Ele existe somente para manutenção do
catálogo de tamanhos de VM e das especificações de hardware.

O script consulta a Azure CLI para os `vm_size` configurados e gera CPU
e memória em formato YAML, facilitando a atualização de
`azure_vm_specs`.

Fluxo:

``` text
lista tamanhos_desejados
        │
        ▼
az vm list-sizes
        │
...
-   `PyYAML`
-   Azure CLI instalada
-   sessão autenticada com `az login`
-   acesso à subscription utilizada
-   permissão para consultar os tamanhos de VM
-   região configurada no script (`brazilsouth` por padrão)

### Execução

A partir da raiz Azure:

``` bash
python scripts/coleta_azure.py
```

Também é possível informar outra região:

``` bash
python scripts/coleta_azure.py --location eastus
```

O arquivo gerado terá estrutura semelhante a:
...
### Relação com `modelo_servidor_map_azure.yml`

O arquivo de variáveis mantém informações com finalidades diferentes:

``` text
modelo_servidor_map
vm_size → objectKey GDA-* do Modelo do Servidor no CMDB

azure_vm_specs
vm_size → cpu + memory_gb
```

O `coleta_azure.py` auxilia somente na geração das especificações de CPU
e memória. Ele **não descobre o objectKey GDA** e **não modifica
automaticamente** o mapeamento do CMDB.

Quando um novo `vm_size` aparecer no inventário, valide:

1.  se existe em `modelo_servidor_map`;
2.  se existe em `azure_vm_specs`;
3.  se o modelo correspondente já existe no Object Type 406 do CMDB.

> Importante: durante o sync normal nenhuma consulta à Azure é feita. A
> Azure CLI é usada somente na execução manual deste script de
> manutenção.
...
## Troubleshooting
  ...
  --------------------------- -------------------------------------------
  CPU não aparece             virtual_machine_size ausente de
                              azure_vm_specs

  Memória não aparece         virtual_machine_size ausente de
                              azure_vm_specs
